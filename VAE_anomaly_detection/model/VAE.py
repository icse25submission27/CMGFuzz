...
class VAEAnomalyDetection(pl.LightningModule, ABC):
    ...
    def is_anomaly(self, x: torch.Tensor, alpha: float = 0.3) -> torch.Tensor:
        """
    ...
    def reconstructed_probability(self, x: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            pred = self.predict(x)
        recon_dist = Normal(pred['recon_mu'], torch.clamp(pred['recon_sigma'], min=1e-7))
        
        x = x.unsqueeze(0)
        p = recon_dist.log_prob(x).exp().mean(dim=0).mean(dim=-1).mean(dim=-1).mean(dim=-1)  # vector of shape [batch_size]
        return p
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_kl', loss['kl'], on_step=True, on_epoch=True)
        self.log('val_recon_loss', loss['recon_loss'], on_step=True, on_epoch=True)
...
class VAEAnomalyTabular(VAEAnomalyDetection):
    ...
    def make_encoder(self, input_size, latent_size):
        ...
        # )
        return nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=4, stride=2, padding=1),  # 输出尺寸: 112x112
            nn.ReLU(),
            nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1),  # 输出尺寸: 56x56
            nn.ReLU(),
            nn.Conv2d(128, 256, kernel_size=4, stride=2, padding=1),  # 输出尺寸: 28x28
            nn.ReLU(),
            nn.Conv2d(256, 512, kernel_size=4, stride=2, padding=1),  # 输出尺寸: 14x14
            nn.ReLU(),
            nn.Conv2d(512, 1024, kernel_size=4, stride=2, padding=1),  # 输出尺寸: 7x7
            nn.ReLU(),
            nn.Flatten(),  # 展平层
            nn.Linear(1024*7*7, latent_size * 2)  # 输出潜在空间的均值和对数方差
        )
    ...
    def make_decoder(self, latent_size, output_size):
        ...
        # )
        return nn.Sequential(
            nn.Linear(latent_size, 1024*7*7),
            nn.ReLU(),
